...
// ----------------- Helper -----------------
const marker = "\u700B";
const withMarker = (text) => text + marker;
const firstSession = {}; // track first-time trigger

function boxReply(name, text) {
  return `╭──────•◈•──────╮
    ʜᴇʏ ɪᴀᴍ ᴛᴏʀᴜ ᴄʜᴀɴ
...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "rX Abdullah",
  description: "AI auto teach with Teach & List support + Box style trigger reply + Typing effect",
...
module.exports.handleEvent = async function ({ api, event, Users }) {
  ...
  if (triggers.includes(text)) {
    ...
    const boxed = boxReply(senderName, replyText);
    ...
    return api.sendMessage(withMarker(boxed), event.threadID, event.messageID);
  }

  // ----------------- Prefix trigger "baby something" -----------------
